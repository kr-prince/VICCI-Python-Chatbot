...
class BotModel(object):
    ...
    def initialize(self):
        ...
        assert os.path.isfile(self.lbencoder_path), "LabelEncoder model file not found"
        assert os.path.isfile(self.train_file_path), "Training data file not found"
        ...
        self.tfidf_dict = dict(zip(self.tfidf.get_feature_names(), list(self.tfidf.idf_)))
        training_data = self.__load_train_data()
        self.query_response = {td['intent']:td['response'] for td in training_data}
    ...
    def __load_glove_vectors(self):
        ...
        if not os.path.isfile(self.glove_path):
            raise Exception("Glove vectors file not found: %s" %self.glove_path)

        with open(self.glove_path) as gfile:
            for line in gfile:
                values = line.split()
    ...
    def __load_train_data(self):
        ...
        if not os.path.isfile(self.train_file_path):
            raise Exception("Training data file not found: %s" %self.train_file_path)
        ...
        for query in queries:
            ...
            for tokn in tokens:
                if tokn in self.embeddings_index and tokn in self.tfidf_dict:
                    ...
                    score = self.tfidf_dict[tokn] * ((tokens.count(tokn)/len(tokens))+1)
                    query_vec += (vec * score)
                    weight_sum += score
                else:
                    pass
    ...
    def train(self, save_models=False):
        ...
        try:
            ...
            queries_train, queries_test, intents_train, intents_test = train_test_split(queries,
                        intents, train_size=0.7, random_state=self.RANDOM_SEED, stratify=intents)
            ...
            print("Train accuracy : %.3f" %(accuracy_score(Y_train, self.bot_clf.predict(X_train))))
    ...
    def response(self, query):
        ...
        try:
            if not self.initialized:
                raise Exception("First initialize the BotModel by running .initialize() method")
...
if __name__ == "__main__":
    ...
    for inp in inputs:
        tag, conf, resp = model.response(inp)
        print(inp," - ",tag," - ",round(conf, 3))
        print(resp)
